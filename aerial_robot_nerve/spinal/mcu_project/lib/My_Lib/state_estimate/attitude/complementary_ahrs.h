...
class ComplementaryAHRS : public EstimatorAlgorithm {
 ...
 private:
  ...
  void estimation() {
    ...
    /* acc correction */
    if (valid_acc == 1 && cnt == 0) est_g_ = (est_g_b_tmp * GYR_CMPF_FACTOR + acc_) * INV_GYR_CMPF_FACTOR;
    ...
    ap::Matrix3f rot =
        ap::Matrix3f(wx_b, wy_b, wz_b);  // rotation of body frame w.r.t. the world frame. Transpose from row to col
